<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否离校" prop="shifoulixiao">
          <el-select v-model="ruleForm.shifoulixiao" placeholder="请选择是否离校">
            <el-option
                v-for="(item,index) in shifoulixiaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否前往疫情中高风险地区" prop="shifouqianwangyiqingzhonggaofengxiandiqu">
          <el-select v-model="ruleForm.shifouqianwangyiqingzhonggaofengxiandiqu" placeholder="请选择是否前往疫情中高风险地区">
            <el-option
                v-for="(item,index) in shifouqianwangyiqingzhonggaofengxiandiquOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
	...
	shhf : false,
      ...
      ruleForm: {
        ...
        shhf: '',
          ...
          shifoulixiaoOptions: [],
          shifouqianwangyiqingzhonggaofengxiandiquOptions: [],
      rules: {
          jiaoshigonghao: [
          ],
          jiaoshixingming: [
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xueyuan!=''&&json.xueyuan){
          ...
          this.$message.error(data.msg);
            ...
            this.shifoulixiaoOptions = "是,否".split(',')
            this.shifouqianwangyiqingzhonggaofengxiandiquOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jiaoshiqingjiashenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiaoshiqingjiashenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiaoshiqingjiashenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
